...
-- Fetch the PlayerModule to read mobile joystick input vectors
local PlayerModule
local OK, Err = pcall(function()
    PlayerModule = require(localPlayer.PlayerGui:WaitForChild("TouchGui", 2) and localPlayer.PlayerScripts:WaitForChild("PlayerModule"))
end)
...
mainFrame.Visible = false -- Kept hidden until opened via the mobile button
...
-- Custom Dragging Implementation (Touch Supported)
...
local function scanForTarget()
    ...
    if searchString == "random" then
        ...
        statusLabel.Text = "Random Loop Mode Enabled"
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= localPlayer then
            ...
            if string.find(userName, searchString) or string.find(displayName, searchString) then
                ...
                statusLabel.Text = "Target Locked: " .. p.DisplayName
...
tpExecuteBtn.Size = UDim2.new(0.5, 0, 1, 0)
...
local function executeTeleport()
    ...
    if teleportState.IsRandomMode then
        local targetPool = {}
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= localPlayer then table.insert(targetPool, p) end
        end
        if #targetPool > 0 then targetPlayer = targetPool[math.random(1, #targetPool)] end
    end
...
local function applyESP(player)
    ...
    local function setupHighlight(character)
        ...
        for _, obj in ipairs(character:GetChildren()) do
            if obj.Name == "CapperESP" or obj.Name == "CapperESPTag" then obj:Destroy() end
        end
    ...
    player.CharacterAdded:Connect(function(char) task.spawn(setupHighlight, char) end)
end
for _, p in ipairs(Players:GetPlayers()) do applyESP(p) end
Players.PlayerAdded:Connect(applyESP)

-- ESP Dynamic Distance Thread
task.spawn(function()
                    ...
                    if targetRoot and billboard then
                        ...
                        if textLabel then
                            if localRoot then
                                ...
                                textLabel.Text = p.DisplayName .. " [" .. distance .. "m]"
                            else
